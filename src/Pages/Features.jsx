import { CgProfile } from "react-icons/cg";
import { FaArrowAltCircleUp, FaRegClock } from "react-icons/fa";
import { SiCashapp } from "react-icons/si";
import Layout from "../Components/Layout";

const features = [
    {
        name: "Get Upskilled",
        description:
            "The #1 way college students & early graduates get Internships, Jobs",
        icon: FaArrowAltCircleUp,
    },
    {
        name: "Create Profile",
        description:
            "Create your profile with easy-to fill form and get recommended with relevant internships/ jobs.",
        icon: CgProfile,
    },
    {
        name: "Hassle Free",
        description:
            "An effortless way to get internships/ jobs through a quick and easy application process.",
        icon: FaRegClock,
    },
    {
        name: "Earn Money",
        description:
            "ou can earn ~ ₹90000 annually through our platform while working as an intern.",
        icon: SiCashapp,
    },
];

export default function Features() {
    return (
        <Layout>
            <div className="bg-white py-10">
                <div className="mx-auto max-w-7xl px-6 lg:px-8">
                    <div className="mx-auto max-w-2xl lg:text-center">
                        <h2 className="text-base font-semibold leading-7 text-indigo-600">
                            InternOasis
                        </h2>
                        <p className="mt-2 text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
                            Everything you need to get your first InternShip
                        </p>
                        <p className="mt-6 text-lg leading-8 text-gray-600">
                            These are the features of our websites
                        </p>
                    </div>
                    <div className="mx-auto mt-16 max-w-2xl sm:mt-20 lg:mt-24 lg:max-w-4xl">
                        <dl className="grid max-w-xl grid-cols-1 gap-x-8 gap-y-10 lg:max-w-none lg:grid-cols-2 lg:gap-y-16">
                            {features.map((feature) => (
                                <div
                                    key={feature.name}
                                    className="relative pl-16"
                                >
                                    <dt className="text-base font-semibold leading-7 text-gray-900">
                                        <div className="absolute left-0 top-0 flex h-10 w-10 items-center justify-center rounded-lg bg-indigo-600">
                                            <feature.icon
                                                className="h-6 w-6 text-white"
                                                aria-hidden="true"
                                            />
                                        </div>
                                        {feature.name}
                                    </dt>
                                    <dd className="mt-2 text-base leading-7 text-gray-600">
                                        {feature.description}
                                    </dd>
                                </div>
                            ))}
                        </dl>
                    </div>
                </div>
            </div>
        </Layout>
    );
}
